...
import { _decorator, Component, game, sys, ImageAsset, Color, Node } from 'cc';
...
interface ISharePopupData {
  result: IPingPangResult;
  userInfo: IUserInfo;
}
...
export class SharePopup extends UiBase {
  ...
  public show(data: ISharePopupData): void {
    console.log(data);
  ...
  /**
     ...
     */
  async generateImage(result: any, userInfo: any): Promise<void> {
    ...
    if (!this._canvas) {
      this._canvas = document.createElement('canvas');
      this._canvas.width = POSTER_W;
      this._canvas.height = POSTER_H;
    } else {
    ...
    ctx.fillStyle = '#217FFF';
    ctx.font = 'bold 38px sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(this._ellipsis(nickname, 12), POSTER_W / 2, 650);
  ...
  showImage() {
    if (this._canvas) {
      ...
      img.src = this._canvas.toDataURL('image/png', 0.8);
      img.onload = () => {
