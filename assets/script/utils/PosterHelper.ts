...
/**
 * H5 海报生成工具
 * - 离屏 Canvas 合成背景 + 头像 + 分数等
 * - 移动端弹窗展示让用户长按保存，PC 端直接下载
 */
export class PosterHelper {
   ...
   */
  static async generate(result: IPingPangResult, userInfo: IUserInfo | null): Promise<void> {
  ...
  private static _loadImg(src: string): Promise<HTMLImageElement | null> {
  ...
  /** 绘制圆形头像，失败时绘制灰色占位 */
  private static async _drawRoundAvatar(
  ...
  /** 文本超长截断 */
  private static _ellipsis(str: string, maxLen: number): string {
   ...
   */
  private static _triggerDownload(dataUrl: string) {
